...
test("__TEST_S400__ auto-scroll cards use an intrinsic first pass and a measured finite viewport", () => {
  const shortCard = resolveDescriptionViewportSizing({
    allocatedCardHeight: 500,
    naturalCardHeight: 180,
    fixedCardHeight: 100,
    descriptionContentHeight: 80,
    minimumViewportHeight: 18,
  });
  assert.deepEqual(shortCard, { shouldBound: false, viewportMaxHeight: null });
  const overflowingDescription = resolveDescriptionViewportSizing({
    allocatedCardHeight: 300,
    naturalCardHeight: 480,
    fixedCardHeight: 150,
    descriptionContentHeight: 330,
    minimumViewportHeight: 18,
  });
  assert.deepEqual(overflowingDescription, { shouldBound: true, viewportMaxHeight: 150 });
});
...
test("__TEST_S400__ Full and Now/Next use the same finite measured description budget", () => {
  const sizing = resolveDescriptionViewportSizing({
    allocatedCardHeight: 300,
    naturalCardHeight: 500,
    fixedCardHeight: 220,
    descriptionContentHeight: 280,
    minimumViewportHeight: 18,
  });
  assert.equal(sizing.shouldBound, true);
  assert.equal(sizing.viewportMaxHeight, 80);
});
